...
public class Solution {
    ...
    static String lcs(int[][] C, char[] aStr, char[] bStr, int x, int y) {
        if (x <= 0 || y <= 0) {
            return "";
        }
        if (aStr[x - 1] == bStr[y - 1]) {// x-1, y-1
            return lcs(C, aStr, bStr, x - 1, y - 1) + aStr[x - 1]; // x-1
        }
        if (C[x][y - 1] > C[x - 1][y]) {
            return lcs(C, aStr, bStr, x, y - 1);
        }
        return lcs(C, aStr, bStr, x - 1, y);
    }
    ...
    static int commonChild(String s1, String s2) {
        ...
        return lcs(result, c1, c2, c1.length, c2.length).length();
    }
